...
function toggleNavMenu() {
  document.getElementById('nav-tabs')?.classList.toggle('nav-open');
}
function closeNavMenu() {
  document.getElementById('nav-tabs')?.classList.remove('nav-open');
}
...
async function boot() {
  ...
  // טען מ-localStorage מיידית — עם הגנה מלאה
  try {
  ...
  if (!state.catalog || !state.catalog.length) state.catalog = [...DEFAULT_CATALOG];
  ...
  try {
    ...
    // הצג שגיאה גלויה במקום מסך ריק
    const c=document.getElementById('catalog-table-container');
    if(c) c.innerHTML='<div style="padding:20px;color:#dc2626">שגיאת תצוגה: '+e.message+'<br>נסה לרענן (משוך מטה)</div>';
  }
  ...
  filtered.forEach(item=>{
    ...
    thtml+='<tr class="catalog-row'+selClass+'">'
      ...
      +'<td style="cursor:pointer" onclick="openItemDetailModal(\''+item.id+'\')">'
        ...
        +'<div class="mobile-price" style="color:#1e3a8a;font-size:.8rem;margin-top:3px;direction:ltr">'+fmtPrice(item.listPrice)+'</div>'
      +'</td>'
